...
test-3:
	...
	vagrant ssh -c "docker run -ti -w /test_dir -v /workspace/sighup/fury-kubernetes-monitoring:/test_dir -v /workspace:/workspace -e CLUSTER_HOST=localhost -e KUBECONFIG=/workspace/kubeconfig --network=host sighup/kind:v1.12.3-bats \"bats -p katalog/tests/tests.bats\"" $(FURY_VM_ID)
